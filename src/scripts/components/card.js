export class Card {
  ...
  _checkIdCard(id) {
    if (id === this._myId) {
      ...
      this._cardElement.content
        ...
        .classList.remove("element__delete_visible"); //удаления свойства visible у корзинки карточки
    }
  }
  showDeleteButtonCard() {
    this._cardElement.content
      .querySelector(".element__delete")
      .classList.add("element__delete_visible");
  }
  ...
  _likeLoaderWrapper = (dataCard, element, checkMyLike) => {
    this._handleLikeClick(dataCard, element, checkMyLike).then(({ likes }) => {
      this._likes = likes;
    });
  };
